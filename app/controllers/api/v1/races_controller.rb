  ...
  def index # rubocop:disable AbcSize, MethodLength, CyclomaticComplexity, PerceivedComplexity
    ...
    unless params[:distance_or_year].blank?
      if "overview".casecmp(params[:distance_or_year]).zero?
        results = Rails.cache.fetch(format(CacheKeys::RACES_OVERVIEW, athlete_id: athlete_id)) do
          items = Race.find_all_by_athlete_id(athlete_id)
        ...
        results = Rails.cache.fetch(format(CacheKeys::RACES_RECENT, athlete_id: athlete_id)) do
          items = Race.find_all_by_athlete_id(athlete_id)
          shaped_items = ApplicationHelper::Helper.shape_races(
            items, heart_rate_zones, measurement_preference
          )
        ...
        results = Rails.cache.fetch(format(CacheKeys::RACES_YEAR, athlete_id: athlete_id, year: year)) do
          items = Race.find_all_by_athlete_id_and_year(athlete_id, year)
        ...
        if results.blank? # Return 404 if nothing found for this year.
          Rails.logger.warn("Could not find requested race year '#{year}' for athlete '#{athlete_id}'.")
        ...
        if race_distance.nil?
          Rails.logger.warn("Could not find requested race distance '#{distance}' for athlete '#{athlete_id}'.")
        ...
        results = Rails.cache.fetch(format(CacheKeys::RACES_DISTANCE,
                                           athlete_id: athlete_id, race_distance_id: race_distance.id)) do
          items = Race.find_all_by_athlete_id_and_race_distance_id(athlete_id, race_distance.id)
          ApplicationHelper::Helper.shape_races(items, heart_rate_zones, measurement_preference)
        end
      end
    end
    render json: results
